...
def read_exp(tokens):
    ...
    def lst_to_linked(lst):
        if lst == []:
            return empty
        return link(lst[0], lst_to_linked(lst[1:]))
    if tokens == []:
    ...
    if token == ')':
        ...
        for i in range(len(rest) + 1):
            if rest[i] == '(': 
                return read_until_close(rest[i + 1:])
            elif rest[i] == ')': 
                return lst_to_linked(new_lst), rest[i + 1:] 
            elif rest[i] != ')':
                new_lst += [numberize(rest[i])]
    else:   
        return read_until_close(tokens)
...
def read_until_close(tokens):
    ...
    token = tokens[0]
    if token == '(': 
        return read_exp(tokens)
    for i in range(len(tokens) + 1):
        if tokens[i] != ')': 
            new_lst += numberize(tokens[i])
        else: 
            return lst_to_linked(new_lst), tokens[i + 1:]
...
def calc_eval(exp):
    ...
    15
    """
    if type(exp) is type(5): 
        return exp 
    elif is_linked_list(first(exp)):
        return calc_eval(first(exp))
    else: 
        return calc_apply(first(exp), rest(exp)) 
