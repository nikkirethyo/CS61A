...
def extract_words(text):
    ...
    for characters in text:
        if characters in ascii_letters or characters == " ":
...
def sentiment_value(s):
    ...
    if make_sentiment(s) == "no sentiment":
        return None 
    else: 
...
def analyze_tweet_sentiment(tweet):
    ...
    for words in words_in_tweet:
        if has_sentiment(get_word_sentiment(words)):
            total += sentiment_value(get_word_sentiment(words))
            counter += 1
            no_sentiment = False 
...
def find_centroid(polygon):
    ...
    [1.0, 2.0, 0.0]
    """
    total_area, lat_total, lon_total = 0, 0, 0
    for i in range(len(polygon) - 1):
        sub_area = (latitude(polygon[i]) * longitude(polygon[i + 1])) - (latitude(polygon[i + 1]) * longitude(polygon[i]))
        total_area += sub_area
        lat_total += (latitude(polygon[i]) + latitude(polygon[i + 1])) * sub_area
        lon_total += (longitude(polygon[i]) + longitude(polygon[i + 1])) * sub_area
    if total_area == 0:
        return (latitude(polygon[0]), longitude(polygon[0]), 0)
    total_area /= 2
    lat_total /= (6 * total_area)
    lon_total /= (6 * total_area)

    return (lat_total, lon_total, abs(total_area))
...
def find_state_center(polygons):
    ...
    for polygon in polygons: 
        ...
        lat_total += centroid[0] * centroid[2]
        lon_total += centroid[1] * centroid[2]
    lat_total /= total_area
    lon_total /= total_area
    return make_position(lat_total, lon_total)
...
def group_tweets_by_state(tweets):
    ...
    def nearest_state(tweet, us_states):
        current_pos = tweet_location(tweet)
        smallest_dist = 0 
        tweet_state = ''
        for state in get_keys(us_states):
            state_center = find_state_center(get_value_from_key(us_states, state))
            distance = geo_distance(state_center, current_pos)
            if smallest_dist == 0:
        ...
        return tweet_state
    for tweet in tweets: 
        state = nearest_state(tweet, us_states)
        if state in get_keys(tweets_by_state):
            prev_tweet = get_value_from_key(tweets_by_state, state)
            tweets_by_state = add_value(tweets_by_state, state, [tweet] + prev_tweet)
        else: 
...
def average_sentiments(tweets_by_state):
    ...
    """
    avg_sentiments_db = make_database()
    for state in get_keys(tweets_by_state): 
        total, counter = 0, 0       
        for tweets in get_value_from_key(tweets_by_state, state): 
            if has_sentiment(analyze_tweet_sentiment(tweets)):
                total, counter = total + sentiment_value(analyze_tweet_sentiment(tweets)), counter + 1
        if counter != 0: 
            avg_sentiments_db = add_value(avg_sentiments_db, state, (total/counter))
    return avg_sentiments_db
