...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if insect.container == True and insect.name == 'Bodyguard':
                self.ant = insect.ant 
            else:
                self.ant = None
        insect.place = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            if self.max_range >= counter and self.min_range <= counter and current_place.bees != []:
    ...
    def action(self, colony):
        if self.place.bees != []: 
            if self.digesting == 0: 
                ...
                self.eat_bee(random_or_none(target)) 
            self.digesting -= 1
        return None      
...
def apply_effect(effect, bee, duration):
    ...
    action, new_action = bee.action, effect(bee.action)
